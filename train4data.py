...
_model_id=sys.argv[2] #获取模型id
_mask_model_id=sys.argv[3] #获取mask_model
if_analysis_sentence=sys.argv[4] #获取是否分析句子
perturbations=sys.argv[5] #获取扰动次数
label=sys.argv[6] #获取标签(ai for 1, human for 0)
env=sys.argv[7] #获取情景

recover=int(sys.argv[8]) #从上次训练失败的地方继续开始训练,default=0
...
values=[]
texts,language = read_texts_from_file(textPath)
print(len(texts))
values.append(textPath.split("\\")[-1])
if(language):
    model_cn=PPL_LL_based_cn(model_id=_model_id)
    for i in range(recover,len(texts)):
        print("cn")
        print("第",i+1,"次预测")
        print(texts[i])
        results=model_cn(texts[i],mask_model_id=_mask_model_id,if_analysis_sentences=int(if_analysis_sentence),n_perturbations=int(perturbations))
    ...
    for i in range(recover,len(texts)):
        ...
        results = model_en(texts[i],model_id=_model_id,mask_model_id=_mask_model_id,n_perturbations=int(perturbations))
