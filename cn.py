...
class PPL_LL_based_cn:
    ...
    def perturb_texts_(self,texts, span_length, pct, ceil_pct=False):
            ...
            if self.mask_model_id=="blueLM":
                ...
                fills = self.deepseekGenMasks(masked_texts,num_masks)
            else:
    ...
    def __call__(self, text,if_analysis_sentences,mask_model_id="blueLM",n_perturbations=15):
        ...
        if if_analysis_sentences==1:
            ...
            for sentence in sentences:
                ...
                pll=np.array(pll)
                pll=self.remove_furthest_two(ll,pll)
            ...
            end_time=time.time()
            for ll,dll,score,ppl,length,ppl_per_line in zip(LL_Per_line,DLL_Per_line,Score_Per_line,PPL_Per_line,length_Per_line,PPL_Per_line):
